...
def prepHour(day,hour):
    ...
    for i in range(len(tw)):
        features = {}
        for x in range(100):
            features['phChange' + str(x)] = pcFeat
            features['phVolume' + str(x)] = vFeat
...
def compileTweetTokenFeats(t, f):
    ...
    for x in range(100):
        ...
        features['emojiHere' + str(x)] = emojiFeat
        features['multEmojiHere' + str(x)] = multEmojiFeat
        features['tagsHere' + str(x)] = tagFeat
        features['elonTagHere' + str(x)] = elonTag
        features['tweetSentiment' + str(x)] = sent
